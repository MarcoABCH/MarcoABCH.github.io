...
$(window).scroll(function () {
  ...
  $("#bloque2").css({ top: -180 - a / 3 + "px" });
  $("#bisoft").css({ top: -250 + a / 5 + "px" });
  $("#empresas").css({ "margin-left": -100 + a / 3.3 + "%" });
  if (a > $("#bloque2").offset().top) {
    $("#empresas").css({ margin: "auto" });
  }
  $("#logros").css({ "margin-left": 200 - a / 5.7 + "%" });
  if (a > $("#bloque3").offset().top) {
    $("#logros").css({ margin: "auto" });
  }
});
$("nav#botonera ul li a").click(function (b) {
  b.preventDefault();
  var a = $(this).attr("href");
  $(a).animatescroll({ easing: "easeOutBounce", scrollSpeed: 2000 });
});
$.scrollUp({ scrollText: "", scrollSpeed: 2000, easingType: "easeOutBounce" });
$("nav#botonesGrupo ul li").click(function () {
  $("#empresas").animate({ height: 0 }, 500, "easeInBack");
  $("#empresas").animate({ width: 0 }, 500, "easeInBack");
  $("#empresas").animate({ height: 400 }, 900, "easeOutBounce");
  $("#empresas").animate({ width: 770 }, 900, "easeOutBounce");
  var a = $(this).attr("enlace");
  $("#empresas img").hide(300);
  $(a).delay(1000).show(100);
});
